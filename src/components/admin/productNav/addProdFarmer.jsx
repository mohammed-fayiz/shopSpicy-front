...
export default function AddProdFarmer() {
  ...
  const [record, setRecord] = React.useState({});
    ...
    const handleSubmit=(e)=>{
        e.preventDefault()
        try {
            axios.post(`${url}/admin/addprodfarmer`,record)
            .then((res)=>{
    ...
    <Box sx={{ display: 'flex' }}>
      ...
      <AppBar position="fixed" open={open} style={{backgroundColor:'rgb(12, 155, 121)'}}>
        <Toolbar>
          ...
          <Typography variant="h6" noWrap component="div" style={{display:'flex',gap:'20px'}}>
            <h4 style={{fontStyle:'italic'}}>shopSpicy</h4> <p>Admin{' > '}Product{' > '}farmer{' > '}add</p>
          </Typography>
        </Toolbar>
      </AppBar>
      ...
      <Box component="main" sx={{ flexGrow: 1, p: 3 }}>
        ...
        <Box  className="form-box">
            <h3 style={{color:'rgb(6, 112, 75)'}}>Add</h3>
