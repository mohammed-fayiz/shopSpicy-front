...
function WholeDashboard() {
  ...
  React.useEffect(()=>{
    ...
    try {
      axios.get(`${url}/user/wholeuserslist`)
      .then((res)=>{
      ...
      <React.Fragment>
        ...
        <div className='product-div'>
          ...
          <Box component="main" sx={{ flexGrow: 1, p: 3 }}>
            ...
            <TableContainer component={Paper}>
              <Table sx={{ minWidth: 500 }} aria-label="customized table">
                <TableHead style={{backgroundColor:'rgb(12, 155, 121)'}}>
                  <TableRow>
                    <StyledTableCell style={{backgroundColor:'rgb(12, 155, 121)'}}>User Name</StyledTableCell>
                ...
                <TableBody>
                  {userData.filter((it) => it.userType!="admin").map((row) => (
          ...
          <div className="product-div-right">
            <h4>Orders By User's</h4>
            <div style={orderData.filter(order=>order.userId.userType=="retailer")==''?{display:'flex'}:{display:'none'}}>
              <h1><em>No Orders by retailers</em></h1>
            </div>
            <Box sx={{ flexGrow:4 }}>
              <Grid container spacing={{ xs: 2, md: 3 }} columns={{ xs: 1, sm: 4, md: 8 }}>
                {orderData.filter(order=>order.userId.userType=="retailer").map((item) => (
                  <Grid item xs={2} sm={4} md={4} key={item._id} >
                    <div className='grid-box'>
                    <h3 style={{textAlign:'center'}}>{item.userId.username}</h3><hr />
                    <h4>Products</h4> 
                    {
                      cartData.filter(cart=>cart._id==item.cartId).map((it)=>
                      <div key={it._id}>
                        <ul>{it.items.map(pro=>
                    ...
                    <>
                        <p>landmark : <strong>{item.landmark}</strong></p>
                          <p>pincode : <strong>{item.pincode}</strong></p>
                          <p>city : <strong>{item.city}</strong></p>
